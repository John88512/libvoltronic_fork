...
/* Globals*/
volatile sig_atomic_t quit = 0;  /* Flag accessible in signal handler */
...
int main() {
  ...
  signal(SIGQUIT, sig_handler);      /* Catch Ctrl+\ (more reliable than Ctrl+Q) */

  printf("Running... Press Ctrl+C or Ctrl+\\ to quit.\n");
  ...
  while (!quit) {
    ...
    result =
        voltronic_dev_execute(dev, VOLTRONIC_EXECUTE_DEFAULT_OPTIONS, output,
                              lengthOutputBuffer, buffer, sizeof(buffer), 1000);
    printf("Sent %s\n", output);
...
/* Signal handler for exiting on ctrl-C */
void sig_handler(int sig) {

    quit = 1;                          /* Set flag to break loop */

    signal(sig, SIG_DFL);              /* Optional: restore default handler after */
